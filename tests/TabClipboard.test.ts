...
import { copyOpenTabLinks, openClipboardLinks } from "../src/workflows/TabClipboard"
...
describe("copyOpenTabLinks", () => {
  it.effect("writes normalized web tab URLs in order and preserves duplicates", () =>
    Effect.gen(function*() {
  ...
  it.effect("fails without replacing the clipboard when no web tabs exist", () =>
    Effect.gen(function*() {
      ...
      const layer = Layer.mergeAll(
        Layer.succeed(
          Tabs,
          Tabs.of({
            queryUrlCandidates: Effect.succeed([undefined, "chrome://settings"]),
            open: () => Effect.void
          })
        ),
        Layer.succeed(
      ...
      const error = yield* copyOpenTabLinks.pipe(Effect.provide(layer), Effect.flip)
  ...
  it.effect("preserves typed query and clipboard write failures", () =>
    Effect.gen(function*() {
      ...
      const queryFailure = yield* copyOpenTabLinks.pipe(Effect.provide(queryLayer), Effect.flip)
      ...
      const writeFailure = yield* copyOpenTabLinks.pipe(Effect.provide(writeLayer), Effect.flip)
...
describe("openClipboardLinks", () => {
  it.effect("opens valid URLs sequentially and reports skipped and failed entries", () =>
    Effect.gen(function*() {
              ...
              Ref.update(attempts, (urls) => [...urls, url.href]).pipe(
                Effect.andThen(
                  url.hostname === "fail.example"
                    ? Effect.fail(new TabCreateError({ url: url.href, cause: "create rejected" }))
                    : Effect.void
                )
              )
          })
  ...
  it.effect("fails when the clipboard has no valid web URLs", () =>
    Effect.gen(function*() {
      ...
      const error = yield* openClipboardLinks.pipe(Effect.provide(layer), Effect.flip)
